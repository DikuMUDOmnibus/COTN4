...
void do_ride(CHAR_DATA * ch, char *argument)
{
        ...
        if (IS_NPC(victim))
        {
                xprintf(buf, "You ride your skeleton steed to %s!\n\r",
                        victim->short_descr);
                send_to_char(buf, ch);
        }
        if (!IS_NPC(victim))
...
void do_knightarmor(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please specify which piece of unholy armor you wish to make: \n\r"
        ...
        {
                send_to_char
                        ("It costs 150 points of primal to create a piece of unholy armor.\n\r",
                         ch);
                return;
        }
...
void do_unholyrite(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You don't have the mystical energies to make the rite.\n\r",
                         ch);
                return;
        }
        if (ch->hit < ch->max_hit)
        {
                ...
                send_to_char
                        ("You make a blood sacrifice to the god of Death.\n\r",
                         ch);
        }
        if (ch->hit > ch->max_hit)
                ch->hit = ch->max_hit;
...
void do_aura(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("What aura do you wish to activate/deactivate ? [bog/death/fear/might].\n\r",
                         ch);
                return;
        }
        if (!str_cmp(arg, "bog"))
...
void do_gain(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
  {
  if (ch->pcdata->powers[INVOCATION] == 0)
      send_to_char("#0Invocation     #0[#R*****#0]#n\n\r",ch);
  if (ch->pcdata->powers[INVOCATION] == 1)
      send_to_char("#0Invocation     #0[#G*#R****#0]#n\n\r",ch);
  if (ch->pcdata->powers[INVOCATION] == 2)
      send_to_char("#0Invocation     #0[#G**#R***#0]#n\n\r",ch);
  ...
  if (ch->pcdata->powers[INVOCATION] == 5)
      send_to_char("#0Invocation     #0[#G*****#0]#n\n\r",ch);

  if (ch->pcdata->powers[NECROMANCY] == 0)
      send_to_char("#0Necromancy     #0[#R**********#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 1)
      send_to_char("#0Necromancy     #0[#G*#R*********#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 2)
      send_to_char("#0Necromancy     #0[#G**#R********#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 3)
      send_to_char("#0Necromancy     #0[#G***#R*******#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 4)
      send_to_char("#0Necromancy     #0[#G****#R******#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 5)
      send_to_char("#0Necromancy     #0[#G*****#R*****#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 6)
      send_to_char("#0Necromancy     #0[#G******#R****#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 7)
      send_to_char("#0Necromancy     #0[#G*******#R***#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 8)
      send_to_char("#0Necromancy     #0[#G********#R**#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 9)
      send_to_char("#0Necromancy     #0[#G*********#R*#0]#n\n\r",ch);
  if (ch->pcdata->powers[NECROMANCY] == 10)
      send_to_char("#0Necromancy     #0[#G**********#0]#n\n\r",ch);


  if (ch->pcdata->powers[SPIRIT] == 0)
      send_to_char("#0Spirit         #0[#R**********#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 1)
      send_to_char("#0Spirit         #0[#G*#R*********#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 2)
      send_to_char("#0Spirit         #0[#G**#R********#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 3)
      send_to_char("#0Spirit         #0[#G***#R*******#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 4)
      send_to_char("#0Spirit         #0[#G****#R******#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 5)
      send_to_char("#0Spirit         #0[#G*****#R*****#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 6)
      send_to_char("#0Spirit         #0[#G******#R****#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 7)
      send_to_char("#0Spirit         #0[#G*******#R***#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 8)
      send_to_char("#0Spirit         #0[#G********#R**#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 9)
      send_to_char("#0Spirit         #0[#G*********#R*#0]#n\n\r",ch);
  if (ch->pcdata->powers[SPIRIT] == 10)
      send_to_char("#0Spirit         #0[#G**********#0]#n\n\r",ch);
    ...
    return;
        ...
        if (!str_cmp(arg, "necromancy"))    /* most powers over death */
        {
                ...
                {
                        send_to_char
                                ("You have mastered the art of necromancy.\n\r",
                                 ch);
                        return;
                }
                else if (ch->practice <
                         ch->pcdata->powers[NECROMANCY] * 60 + 60)
                {
                        send_to_char
                                ("Your control over the forces of life and death is not powerful enough.\n\r",
                                 ch);
                        return;
                }
                else
                {
                        ch->practice -=
                                ch->pcdata->powers[NECROMANCY] * 60 + 60;
                        send_to_char
                                ("Death and life is yours to command.\n\r",
                                 ch);
        ...
        else if (!str_cmp(arg, "invocation"))   /* attacks like powerwords */
        {
                ...
                {
                        send_to_char
                                ("You have mastered the art of invocation.\n\r",
                                 ch);
                        return;
                }
                else if (ch->practice <
                         ch->pcdata->powers[INVOCATION] * 60 + 60)
                {
                        send_to_char
                                ("You are not ready to advance in your magical studies.\n\r",
                                 ch);
                        return;
                }
                else
                {
                        ch->practice -=
                                ch->pcdata->powers[INVOCATION] * 60 + 60;
                        send_to_char
                                ("Your mastery of the ancient arts increase.\n\r",
                                 ch);
        ...
        else if (!str_cmp(arg, "spirit"))   /* toughness */
        {
                ...
                {
                        send_to_char
                                ("You have completely bound your spirit to this vessel.\n\r",
                                 ch);
                        return;
                }
                if (ch->practice <
                    ch->pcdata->powers[UNDEAD_SPIRIT] * 60 + 60)
                {
                        send_to_char
                                ("You are not ready to bind more of your spirit yet.\n\r",
                                 ch);
                        return;
                }
                else
                {
                        ch->practice -=
                                ch->pcdata->powers[UNDEAD_SPIRIT] * 60 + 60;
                        send_to_char
                                ("You channel more of your spirit from the abyss into this body.\n\r",
                                 ch);
...
void do_weaponpractice(CHAR_DATA * ch, char *argument)
{
        if (IS_NPC(ch))
                return;
        if (!IS_CLASS(ch, CLASS_UNDEAD_KNIGHT) && !IS_CLASS(ch, CLASS_BLADE_MASTER))
        {
                send_to_char("You are not one of the undead!!!\n\r", ch);
                return;
        }
        ...
        {
                send_to_char
                        ("You have already mastered the art of combat.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("You are not ready to train your weaponskill.\n\r",
                         ch);
                return;
        }
        else
        {
                ...
                send_to_char
                        ("You feel your skills with weapons increase as you make your bloodsacrifice.\n\r",
                         ch);
                ch->hit = 1;
                ch->mana = 1;
                ch->move = 1;
