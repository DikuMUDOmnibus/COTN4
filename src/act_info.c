...
char     *format_obj_to_char(OBJ_DATA * obj, CHAR_DATA * ch, bool fShort)
{
    ...
    if (obj->item_type == ITEM_WEAPON)
        {
                sprintf(lvl, "#R[#7%d#R]#n", obj->weapon_level);
                strcat(buf, lvl);
        }
...
int char_damroll(CHAR_DATA * ch)
{
	...
	value += (ch->pcRaceLevel * 5);
        ...
        // Player Focus (Power)
        if (ch->ability_cur == 1)  value += ch->ability[1] * 10;
...
int char_hitroll(CHAR_DATA * ch)
{
	...
	value += ch->symbiotehr;
        ...
        // Player Focus (Skill)
        if (ch->ability_cur == 2)  value += ch->ability[1] * 10;
...
int char_ac(CHAR_DATA * ch)
{
	...
	value -= ch->symbioteac;
        ...
        if (ch->ability_cur == 3)  value -= ch->ability[3] * 10;
	...
	count = 0;
        ...
        for (d = descriptor_list; d != NULL; d = d->next)
        {
			...
			count++;
                 ...
                 * Show Status 
                 */
                sprintf(pkratio, "#L[#7%3d#L]#n", wch->race);
...
*/
					...
					class = "#0Human#n";
                                else if (IS_RACE(wch, RACE_CHIMERA))
                                        class = "Chimera";
                                else if (IS_RACE(wch, RACE_CYCLOPS))
                                        class = "Cyclops";
                                else if (IS_RACE(wch, RACE_GRIFFIN))
                                        class = "Griffin";
                                else if (IS_RACE(wch, RACE_HARPIE))
                                        class = "Harpie";
                                else if (IS_RACE(wch, RACE_PHOENIX))
                                        class = "Phoenix";
                                else if (IS_RACE(wch, RACE_SATYR))
                                        class = "Satyr";
                                else if (IS_RACE(wch, RACE_MEDUSA))
                                        class = "Medusa";
                                else if (IS_RACE(wch, RACE_TITAN))
                                        class = "Titan";
                                else if (IS_RACE(wch, RACE_FENRIS))
                                        class = "Fenris";
                                else if (IS_RACE(wch, RACE_NYMPH))
                                        class = "Nymph";
                                else if (IS_RACE(wch, RACE_SPECTRE))
                                        class = "Spectre";
                                else if (IS_RACE(wch, RACE_MINOTAUR))
                                        class = "Minotaur";
                                else if (IS_RACE(wch, RACE_CENTAUR))
                                        class = "Centaur";
                                else if (IS_RACE(wch, RACE_CERBERUS))
                                        class = "Cerberus";
                                else if (IS_RACE(wch, RACE_VALKYRIE))
                                        class = "Valkyrie";
				else 
					class = "#RNone#n";
                                break;
...
/*
                                xcatf(buf20, "%-12s%s %-28s%-12s %s %s\n\r",
                                      ...
                                      wch->pcdata->switchname, king, faith);
                                        ...
                                        class = "#0B#Rully#n";
*/
                                xcatf(buf20, "%-6s #7%s #0the %s #G%s #R%s %s %s\n\r",
                                      pkratio, wch->pcdata->switchname, sexbuf, class,
                                      kav,// king, faith
					(IS_EXTRA(wch, EXTRA_AFK)) ? "#L[#7AFK#L]#n " : "",
			(IS_SET(wch->pcdata->jflags, JFLAG_BULLY)) ? "#L[#7BULLY#L]#n " : ""
...
/*      else
      {
        sprintf(buf16 + strlen(buf16), " %-6s #7%s #0the %s #G%s#0 #R%s#0 %s %s worships %s\n\r",
        pkratio, wch->pcdata->switchname, sexbuf, title, kav, guild, faith ,god);
        a16 = TRUE; avatars = TRUE;
      }
*/
                else if (wch->level < 3)
                {
                                xcatf(buf20, "%-6s #7%s #0the %s #G%s #R%s\n\r",
                                      pkratio, wch->pcdata->switchname, sexbuf, class,
                                      kav,
                                        (IS_EXTRA(wch, EXTRA_AFK)) ? "#L[#7AFK#L]#n " : "",
                        (IS_SET(wch->pcdata->jflags, JFLAG_BULLY)) ? "#L[#7BULLY#L]#n " : ""
...
//                if (!mains)
//                {
  sprintf(buf, " %s\n\r", generate_banner( "#YThe Rogue Legion: Rebirth", 80, "#0" ) );
                        send_to_char(buf, ch);

/*cent_to_char("#R+---X        x          x  x",ch);
cent_to_char("#0|            |          |  |",ch);
cent_to_char("#Rx     +--+ x-+-x x---x  x--x",ch);
cent_to_char("#0|     |  |   |   |   |     |",ch);
cent_to_char("#R+---X +--+   x   X   x     x#n",ch);
                        mains = TRUE;
  */
//              }
...
//        }
  ...
  sprintf(buf, " %s\n\r", generate_banner( "#YAVATARS", 80, "#0" ) );
        ...
        if (mor)
        {
                ...
                if (!mortal)
                {
  sprintf(buf, " %s\n\r", generate_banner( "#YMORTALS", 80, "#0" ) );
  ...
  b = a / 24;
  c = a - b * 24;
  sprintf(buf, "#R[#0Statistics Reset#R:#Y %d #0days and #Y%d #0hours ago#R]",
  b, c);
  cent_to_char(buf, ch);
  ...
  c = c * 10;
  c = b - c;
  sprintf(buf, "[#0Average Online#R:#Y %d#R][#0Peak Online#R:#Y %d#R][#0Max Ever#R:#Y %d#R]#n",
...
/* Still unsure of why this needs to be here - Tijer */
        ...
        sprintf(buf, " #0It is %d o'clock %s, Day of %s, %d%s the Month of %s.#n",
                (time_info.hour % 12 == 0) ? 12 : time_info.hour % 12,
                time_info.hour >= 12 ? "pm" : "am",
                day_name[day % 7],
                day, suf,
        ...
        cent_to_char(buf, ch);
  sprintf(buf, " %s\n\r", generate_banner( "#Yhttp://www.ianshirm.net#n", 80, "#0" ) );
        send_to_char(buf, ch);
    ...
    if ( its_christmas )
    {
        sprintf ( buf, " #sIT'S #sCHRISTMAS#s!!! #sLETS #sCELEBRATE #sWITH #sALL #sDOUBLES#s!!!#n\n\r" );
        cent_to_char(buf, ch);
    }

    if ( special_day )
    {
        sprintf ( buf, " #YIT'S A SPECIAL DAY TODAY - ALL DOUBLES :)#n\n\r" );
        cent_to_char(buf, ch);
    }
