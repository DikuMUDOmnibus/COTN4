...
void do_ccenter(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0')
        {
                ...
                send_to_char(buf, ch);
                xprintf(buf,
                        "  #R(#0basic#R) #YBasic Load Ratio             #G1#7:#G%d#n\n\r",
                        ccenter[CCENTER_BASIC_LOAD]);
                send_to_char(buf, ch);
                xprintf(buf,
                        "  #R(#0rare#R) #YRare Load Ratio              #G1#7:#G%d#n\n\r",
                        ccenter[CCENTER_RARE_LOAD]);
                send_to_char(buf, ch);
                send_to_char
        ...
        else if (!str_cmp(arg1, "basic"))
        {
                ccenter[CCENTER_BASIC_LOAD] = atoi(arg2);
                send_to_char("Ok. Value Changed.\n\r", ch);
                save_ccenter();
                do_ccenter(ch, "");
                return;
        }
        else if (!str_cmp(arg1, "rare"))
        {
                ccenter[CCENTER_RARE_LOAD] = atoi(arg2);
                send_to_char("Ok. Value Changed.\n\r", ch);
                save_ccenter();
                do_ccenter(ch, "");
                return;
        ...
        else if (!str_cmp(arg1, "reset"))
        {
                ...
                ccenter[CCENTER_BASIC_LOAD] = 1000;
                ccenter[CCENTER_RARE_LOAD] = 8000;
