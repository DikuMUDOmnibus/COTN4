...
void save_char_obj(CHAR_DATA * ch)
{
        ...
        fclose(fpReserve);
/*
 * Save pfile to a temporary location
 */
        xprintf(strsavetmp, "%s/store/%s", PLAYER_DIR,
...
/*
 * Once the pfile has fully saved move to the default playerfile location
 */

        xprintf (strsave, "%s%s", PLAYER_DIR, capitalize (ch->pcdata->switchname));
...
// Symbiote variables
    ...
    fprintf(fp, "Ability   %d %d %d %d %d\n", ch->ability[0],
	    ch->ability[1],ch->ability[2],ch->ability[3],ch->ability[4]);
    fprintf(fp, "Abilityexp   %ld %ld %ld %ld %ld\n", ch->ability_exp[0],
    ch->ability_exp[1],ch->ability_exp[2],ch->ability_exp[3],ch->ability_exp[4]);

    fprintf(fp, "Abilitycur %d %d %d %d %d %d\n", ch->ability_cur[0],
	ch->ability_cur[1],ch->ability_cur[2], ch->ability_cur[3], ch->ability_cur[4],
	ch->ability_cur[5]);
...
void fwrite_obj(CHAR_DATA * ch, OBJ_DATA * obj, FILE * fp, int iNest)
{
        ...
        fprintf(fp, "WeaponLvL     %d\n", obj->weapon_level);
        fprintf(fp, "WeaponTotalxp     %d\n", obj->weapon_totalxp);
...
bool load_char_short(DESCRIPTOR_DATA * d, char *name)
{
    ...
    for (sn = 0; sn < 5; sn++) // ability, by xrak
	ch->ability[sn] = 1;	
    for (sn = 0; sn < 5; sn++) // ability, by xrak
        ch->ability_exp[sn] = 0;
    for (sn = 0; sn < 6; sn++) // ability, by xrak
        ch->ability_cur[sn] = -1;
...
//Familiar stuff - Zarius
        ...
        for (;;)
        {
			...
			KEY("Quest2", obj->quest2, fread_number(fp));
		        ...
		        KEY( "WeaponLvL",   obj->weapon_level,      fread_number(fp));
		        KEY( "WeaponTotalxp",   obj->weapon_totalxp,      fread_number(fp));
...
void fread_char(CHAR_DATA * ch, FILE * fp)
{
        ...
        for (;;)
        {
                ...
                case 'A':
                        ...
                        if (!str_cmp(word, "Ability")) // Ability by Xrakisis
                        {
                                ch->ability[0] = fread_number(fp);
                                ch->ability[1] = fread_number(fp);
                                ch->ability[2] = fread_number(fp);
                                ch->ability[3] = fread_number(fp);
                                ch->ability[4] = fread_number(fp);
                                fMatch = TRUE;
                                break;
                        }
                        if (!str_cmp(word, "Abilityexp")) // Ability by Xrakisis
                        {
                                ch->ability_exp[0] = fread_number(fp);
                                ch->ability_exp[1] = fread_number(fp);
                                ch->ability_exp[2] = fread_number(fp);
                                ch->ability_exp[3] = fread_number(fp);
                                ch->ability_exp[4] = fread_number(fp);
                                fMatch = TRUE;
                                break;
                        }
                        if (!str_cmp(word, "Abilitycur")) // Ability by Xrakisis
                        {
                                ch->ability_cur[0] = fread_number(fp);
                                ch->ability_cur[1] = fread_number(fp);
                                ch->ability_cur[2] = fread_number(fp);
                                ch->ability_cur[3] = fread_number(fp);
                                ch->ability_cur[4] = fread_number(fp);
                                ch->ability_cur[5] = fread_number(fp);
                                fMatch = TRUE;
                                break;
                        }
