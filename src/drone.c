...
void do_visage(CHAR_DATA * ch, char *argument)
{
        ...
        if (ch->pcdata->powers[DRONE_DISTORTED_REALITY] < 1
            || ch->pcdata->powers[DRONE_MINDCONTROL] < 1)
        {
                send_to_char
                        ("You need level 1 in distorted realities and mindcontrol.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("You haven't shifted reality yet, you cannot grow a visage here.\n\r",
                         ch);
                return;
        }
        if (ch->mana < 2000)
...
void do_memorytwist(CHAR_DATA * ch, char *argument)
{
        ...
        if (ch->pcdata->powers[DRONE_ALTERNATE_REALITY] < 3
            || ch->pcdata->powers[DRONE_WILLPOWER] < 1)
        {
                send_to_char
                        ("You need level 3 alternative realities and level 1 willpower.\n\r",
                         ch);
                return;
        }
        if (!IS_SET(ch->pcdata->jflags, JFLAG_DRONEWORLD))
        {
                send_to_char
                        ("You haven't shifted reality yet, you cannot twist the memory of anyone in this reality.\n\r",
                         ch);
                return;
        }
        if (ch->move < 1000)
...
void do_nightmare(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You haven't mastered all aspects of reality.\n\r",
                         ch);
                return;
        }
...
void do_horrid(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You don't have the power to create a horrid reality yet.\n\r",
                         ch);
                return;
        }
        if (ch->mana < 2000)
        ...
        {
                send_to_char
                        ("It lacks the mental abilities to understand a change in reality.\n\r",
                         ch);
                return;
        }
        ...
        act("You weave a horrid reality and let $N be the center of it.", ch,
            NULL, victim, TO_CHAR);
...
void do_distort(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You don't have the power to distort reality yet.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("Your have already distorted reality to suit your needs.\n\r",
                         ch);
                return;
        }
        ...
        act("$n weaves reality into something distorted and perverted.", ch,
            NULL, NULL, TO_ROOM);
...
void do_mindseye(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        (" #G<<<#0- - - - --------========#Y{ #0Drone Mindseye #Y}#0========-------- - - - -#G>>>#n\n\r",
                         ch);
                send_to_char
                        ("    Mindseye allows a drone to force players that looks at him or her\n\r",
                         ch);
                send_to_char
                        ("    to execute a special command, chosen by the Drone itself.\n\r\n\r",
                         ch);
                send_to_char
                        ("    'Readaura' gives an unblockable readable of the person looking.\n\r",
                         ch);
                send_to_char
                        ("    'Level'    gives an insight into the lookers skills.\n\r",
                         ch);
                send_to_char
                        ("    'Custom'   allows for a custom forced command of the drones choice.\n\r",
                         ch);
                send_to_char("    'Off'      turns off the power.\n\r\n\r",
                             ch);
                send_to_char
                        ("    Syntax : mindseye <custom/readaura/off> <custom command>\n\r",
                         ch);
                send_to_char
                        (" #G<<<#0- - - - --------========#Y{ #0Drone Mindseye #Y}#0========-------- - - - -#G>>>#n\n\r",
                         ch);
...
void do_psionicburst(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Your blast can only affect awake and relaxed minds.\n\r",
                         ch);
                return;
        }
        if (is_safe(ch, victim))
                return;
        do_say(ch,
               "#YLook into my eyes and relax, you feel your eyelids growing heavy.#n");
...
void do_realityshift(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You haven't mastered the alternate realities yet.\n\r",
                         ch);
                return;
        }
        if ((victim = ch->fighting) == NULL)
        ...
        {
                send_to_char
                        ("Nay, you cannot create a new reality for mobs.\n\r",
                         ch);
                return;
        }
        if (IS_SET(ch->pcdata->jflags, JFLAG_DRONEWORLD))
        ...
        act("$n creates an alternate reality, sucking $N with $m into it.",
            ch, NULL, victim, TO_NOTVICT);
        act("You create an alternate reality, sucking $N with you into it.",
            ch, NULL, victim, TO_CHAR);
        act("$n creates an alternate reality, sucking you with $m into it.",
...
void do_mindwarp(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You cannot concentrate on them, your already fighting someone else.\n\r",
                         ch);
                return;
        }
        if (ch->mana < 500)
        ...
        if (number_range(1, 3) == 2)
        {
                ...
                act("$N resists your psionic attack.", ch, NULL, victim,
                    TO_CHAR);
        ...
        act("You unleash your most potent psionic attack, paralyzing $N.", ch,
            NULL, victim, TO_CHAR);
        act("You abuses the pause to land a devastating attack.", ch, NULL,
            victim, TO_CHAR);
...
void do_dronearmor(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please specify which piece of drone armor you wish to make: Tentacle Ring\n\r"
        ...
        {
                send_to_char
                        ("It costs 150 points of primal to create drone equipment.\n\r",
                         ch);
                return;
        }
        if (!str_cmp(arg, "tentacle"))
                vnum = 33300;
        else if (!str_cmp(arg, "ring"))
                vnum = 33301;
        else if (!str_cmp(arg, "collar"))
                vnum = 33302;
        else if (!str_cmp(arg, "wristbands"))
                vnum = 33303;
                ...
                vnum = 33308;
        else if (!str_cmp(arg, "paddings"))
                vnum = 33309;
        else if (!str_cmp(arg, "cape"))
                vnum = 33310;
        else if (!str_cmp(arg, "girdle"))
                vnum = 33311;
...
void do_mentalblock(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You need willpower level 3 to create a mental block.\n\r",
                         ch);
                return;
        }
        ...
        send_to_char
                ("You create a mental block, preventing anyone from controlling your actions.\n\r",
                 ch);
        return;
}
