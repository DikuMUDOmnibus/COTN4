...
BOARD_DATA boards[MAX_BOARD] = {
        ...
        {"General", "General discussion", 0, 2, "all", DEF_INCLUDE, 14, NULL,
         FALSE}
        ,
        {"Ideas", "Suggestion for improvement", 0, 2, "all", DEF_NORMAL, 14,
         NULL, FALSE}
        ,
        {"Announce", "Announcements from Immortals", 0, 8, "all", DEF_NORMAL,
         60, NULL, FALSE}
        ,
        {"Bugs", "Typos, bugs, errors", 2, 2, "imm", DEF_INCLUDE, 60, NULL,
         FALSE}
        ,
        {"Personal", "Personal messages", 0, 2, "all", DEF_EXCLUDE, 14, NULL,
         FALSE}
        ,
        {"Immortal", "Immortal Board", 7, 7, "imm", DEF_INCLUDE, 60, NULL,
         FALSE}
        ,
        {"Builder", "Builder Board", 7, 7, "imm", DEF_INCLUDE, 20, NULL, FALSE}
        ,
        {"Kingdom", "Kingdom messages", 0, 2, "all", DEF_EXCLUDE, 20, NULL,
         FALSE}
        ,
        {"Religion", "Religion messages", 0, 2, "all", DEF_EXCLUDE, 21, NULL,
         FALSE}
};

/* The prompt that the character is given after finishing a note with ~ or END */
const char *szFinishPrompt =
        "(" BOLD "C" NO_COLOR ")ontinue, (" BOLD "V" NO_COLOR ")iew, (" BOLD
        "P" NO_COLOR ")ost or (" BOLD "F" NO_COLOR ")orget it?";
...
static void show_note_to_char(CHAR_DATA * ch, NOTE_DATA * note, int num)
{
         ...
         * Ugly colors ? 
         */
        xprintf(buf, "#B[#W%4d#B]#Y %s: #w%s#n\n\r"
                "#YDate:  #0%s\n\r"
                "#YTo:    #W%s\n\r"
                "#b===========================================================================\n\r"
                "#w%s\n\r",
...
static void load_board(BOARD_DATA * board)
{
        ...
        for (;;)
        {
                ...
                if (pnote->expire < current_time)
                {
                        ...
                        xprintf(archive_name, "%s%s.old", NOTE_DIR,
                                board->short_name);
...
static void do_nwrite(CHAR_DATA * ch, char *argument)
{
        ...
        if (!ch->pcdata->in_progress)
        {
                ...
                ch->pcdata->in_progress->sender =
                        str_dup(ch->pcdata->switchname);
        ...
        act(BOLD GREEN "$n starts writing a note." NO_COLOR, ch, NULL, NULL,
            TO_ROOM);
         ...
         */
        xprintf(buf,
                "You are now %s a new note on the " BOLD "%s" NO_COLOR
                " board.\n\r"
        ...
        if (!ch->pcdata->in_progress->text) /* Are we continuing an old note or not? */
        {
                ...
                case DEF_NORMAL:
                        xprintf(buf,
                                "If you press Return, default recipient \""
                                BOLD "%s" NO_COLOR "\" will be chosen.\n\r",
                                ch->pcdata->board->names);
                        break;
                case DEF_INCLUDE:
                        xprintf(buf,
                                "The recipient list MUST include \"" BOLD "%s"
                                NO_COLOR
                                "\". If not, it will be added automatically.\n\r",
                                ch->pcdata->board->names);
                        break;

                case DEF_EXCLUDE:
                        xprintf(buf,
                                "The recipient of this note must NOT include: \""
                                BOLD "%s" NO_COLOR "\".",
                                ch->pcdata->board->names);
        ...
        else    /* we are continuing, print out all the fields and the note so far */
        {
                ...
                send_to_char("\n\rEnter text. Type " BOLD "~" NO_COLOR " or "
                             BOLD "END" NO_COLOR
...
static void do_nremove(CHAR_DATA * ch, char *argument)
{
        ...
        if (str_cmp(ch->pcdata->switchname, p->sender)
            && ch->trust < MAX_LEVEL)
        {
                send_to_char
...
static void do_nlist(CHAR_DATA * ch, char *argument)
{
        ...
        send_to_char("#wNotes on this board#r:#n\n\r"
                     "#BNum#0> #WAuthor        #BSubject#n\n\r", ch);
...
void do_board(CHAR_DATA * ch, char *argument)
{
	...
	char *write;
        ...
        if (!argument[0])   /* show boards */
        {
                ...
                send_to_char("\n\r", ch);
                send_to_char
                        ("#WNum #0Name        #wNew#R/#wAll  #wRead#R/#wWrite  #WDescription#n\n\r",
                         ch);
                send_to_char("#w=== ========== ========= ===========#n\n\r",
                             ch);
                for (i = 0; i < MAX_BOARD; i++)
        ...
        if (ch->pcdata->board->write_level > get_trust(ch))
        write = "W";
        if (ch->pcdata->board->write_level < get_trust(ch))
        write = "-";
                        ...
                        {   /* watch out for the non-portable &%c !!! */
                                xprintf(buf,
                                        "#W%2d#R> #w%-10s#B [#0%3d#R/#0%3d#B] #B[  #0%s#B  ] #w%s#n\n\r",
                                        count, boards[i].short_name, unread,
                                        total_notes(ch, &boards[i]), 
                ...
                {
                        xprintf(buf,
                                "\n\r#wYour current board is #W%s#n.\n\r",
                                ch->pcdata->board->short_name);
                        send_to_char(buf, ch);
                        ...
                        if (ch->pcdata->board->read_level > get_trust(ch))
                                send_to_char
                                        ("#0You cannot read nor write notes on this board.#n\n\r",
                                         ch);
                        else if (ch->pcdata->board->write_level >
                                 get_trust(ch))
                                send_to_char
                                        ("#0You can only read notes from this board.#n\n\r",
                                         ch);
                        else
                                send_to_char
                                        ("#0You can both read and write on this board.#n\n\r",
                                         ch);
                }
                send_to_char("\n\r", ch);
                return;
        ...
        if (is_number(argument))
        {
                ...
                if (count == number)    /* found the board.. change to it */
                {
                        ...
                        xprintf(buf,
                                "Current board changed to " BOLD "%s" NO_COLOR
        ...
        ch->pcdata->board = &boards[i];
        xprintf(buf,
                "Current board changed to " BOLD "%s" NO_COLOR ". %s.\n\r",
                boards[i].short_name,
                (get_trust(ch) <
                 boards[i].
                 write_level) ? "You can only read here" :
...
void handle_con_note_to(DESCRIPTOR_DATA * d, char *argument)
{
        ...
        case DEF_NORMAL:   /* default field */
                if (!buf[0])    /* empty string? */
                {
                        ...
                        xprintf(buf,
                                "Assumed default recipient: " BOLD "%s"
                 ...
                 */
        {
                ...
                if (IS_IMMORTAL(ch))    /* immortals get to choose number of expire days */
                {
                        xprintf(buf,
                                ...
                                "Press Enter for default value for this board, "
                                BOLD "%d" NO_COLOR " days.\n\r" BOLD YELLOW
                                "Expire" NO_COLOR ":  ",
                ...
                else
                {
                        ...
                        xprintf(buf, "This note will expire %s\r",
                                ctime(&ch->pcdata->in_progress->expire));
                        write_to_buffer(d, buf, 0);
                        write_to_buffer(d,
                                        "\n\rEnter text. Type " BOLD "~"
                                        NO_COLOR " or " BOLD "END" NO_COLOR
                                        " on an empty line to end note.\n\r"
                                        "=======================================================\n\r",
                                        0);
...
void handle_con_note_expire(DESCRIPTOR_DATA * d, char *argument)
{
        ...
        write_to_buffer(d,
                        "\n\rEnter text. Type " BOLD "~" NO_COLOR " or " BOLD
                        "END" NO_COLOR " on an empty line to end note.\n\r"
                        "=======================================================\n\r",
                        0);
...
void handle_con_note_finish(DESCRIPTOR_DATA * d, char *argument)
{
        ...
        case 'v':  /* view note so far */
                ...
                {
                        write_to_buffer(d,
                                        GREEN "Text of your note so far:"
                                        NO_COLOR "\n\r", 0);
        ...
        case 'p':  /* post note */
                if (board_number(ch->pcdata->board) < 4
                    && is_full_name("all", ch->pcdata->in_progress->to_list))
                {
                        xprintf(buf,
                                "A new note has been posted by %s on board %d",
                                ch->name,
                                board_number(ch->pcdata->board) + 1);
                        do_info(ch, buf);
                }
                ...
                d->connected = CON_PLAYING;
                ch->pcdata->in_progress = NULL;
                act(BOLD GREEN "$n finishes $s note." NO_COLOR, ch, NULL,
                    NULL, TO_ROOM);
                break;
        case 'f':
