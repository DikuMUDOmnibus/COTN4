...
void do_mudstat(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("#R*** #7Double Practice points #GON#n\n\r", ch);
        if (double_pracs == FALSE)
        send_to_char("#R*** #7Double Practice points #ROFF#n\n\r", ch);

        if (global_dt)
        {
                send_to_char("#R*** #7SUPER TRAINING is #GON!#n\n\r#n", ch);
                if (pulse_dt > 0)
                {
                        xprintf(buf,
                                "      #Y%d #0minutes remaining#n\n\r\n\r",
        ...
        {
                send_to_char("#R*** #WSUPER TRAINING is #ROFF#n\n\r#n", ch);
                xprintf(buf,
